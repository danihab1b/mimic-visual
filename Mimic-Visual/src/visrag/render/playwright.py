"""Playwright renderer — screenshot capture via Playwright."""
...
class PlaywrightRenderer(AbstractRenderer):
    async def render(
        ...
        try:
            ...
            raise RuntimeError("Playwright required. pip install playwright")
        ...
        async with async_playwright() as p:
            ...
            if document.source.value == "web":
                ...
                await browser.close()
                raise ValueError(f"Playwright renderer cannot handle: {document.path}")
        ...
        meta = {"url": document.path} if document.source.value == "web" else {}
